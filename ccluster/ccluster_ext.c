...
cluster_kmeans(PyObject *module, PyObject *args)
{
    ...
    PyObject *py_new_partition, *callback_result;
    ...
    py_new_partition = PyArray_NewLikeArray((PyArrayObject*)py_partition, NPY_ANYORDER,
                                            NULL, 0);
    ...
    /* Check for callback. If no callback was provided, release GIL */
    if (callback != NULL) {
    ...
    while (swaps > swap_threshold)
    {
        ...
        #pragma omp for private(j,l,min_dist,min_dist_index,dist,cindex,pindex,d)
        for (i=0; i < points_shape[0]; i++) {
            min_dist = DBL_MAX;
            min_dist_index = -1;
            for (j=0; j < clusters_shape[0]; j++) {
                dist = 0.0;
                for (l=0; l < clusters_shape[1]; l++) {
                    cindex = j*clusters_shape[1] + l;
                    pindex = i*points_shape[1] + l;
